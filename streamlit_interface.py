...
st.markdown("Загрузите техническое задание, прайс-листы и (опционально) файл со скидками — система всё сделает сама.")
...
if st.button("🚀 Запустить подбор"):
    if uploaded_spec and uploaded_prices:
        with st.spinner("⚙️ Обработка данных..."):
            try:
                ts_text, result_df, result_file = process_documents(
                    spec_file=uploaded_spec,
                    prices_files=uploaded_prices,
                    discounts_file=uploaded_discounts
                )
        ...
        st.warning("⚠️ Пожалуйста, загрузите ТЗ и хотя бы один прайс.")
